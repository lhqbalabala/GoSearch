...
func Compression(data []byte) []byte {
	...
	write.Write(data)
	write.Flush()
	fmt.Println("原大小：", len(data), "压缩后大小：", buf.Len(), "压缩率：", fmt.Sprintf("%.2f", float32(buf.Len())*100/float32(len(data))), "%")
	return buf.Bytes()
}
...
func Read(data interface{}, filename string) {
	...
	err = dec.Decode(data)
	if err != nil {
		fmt.Println("Decode Error: ", err, "buffer.Bytes() is :", buffer.Bytes())
	}
}
